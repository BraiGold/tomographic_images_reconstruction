...
tiemposTotalesRala = []
tiemposCMLRala = []
for nDiscret in range(5,35,5):
    vecEntradas.append(nDiscret)
    ...
    archivoTiempoTotal.close()
    archivoTiempoCML.close()
    # empieza RALA
    subprocess.call(['./main','-r',str(ruido), '-i' ,csvEntrada, '-s',csvSalida,'tipoRayo',str(4),'transpongoONO',str(trasponerONo),'usoMatrizEsparsaONO',str(1),'-n',str(nDiscret)])
    archivoTiempoCMLRala = open('./archivosGenerados/tiempoEjecucionCML.txt','r')
    tiempoCML = float(archivoTiempoCMLRala.readline())
    archivoTiempoTotalRala = open('./archivosGenerados/tiempoEjecucionTotal.txt','r')
    tiempoTotal = float(archivoTiempoTotalRala.readline())
    tiemposTotalesRala.append(tiempoTotal)
    tiemposCMLRala.append(tiempoCML)




#Segundo grafico : tiempos
plt.plot(vecEntradas,tiemposTotales,'r-',label='implementacion vector<vector<double> >')
#plt.plot(vecEntradas,tiemposCML,'r--',label='cuadrados minimos lineales')
plt.plot(vecEntradas,tiemposTotalesRala,'b-',label='implementacion matriz rala')
#plt.plot(vecEntradas,tiemposCMLRala,'b--',label='CML RALA')
plt.legend()
plt.xlabel("n-discretización")
plt.ylabel("Tiempo (seg)")
plt.show()
print("final finaloso 2")
