...
#este mismo codigo funciona para el experimento 5 y para graficar tiempos en fucion de rayos experimento 3
# para esto es necesario alternar entre los bloques de codigo comentados al final (los plots)
...
def generarPuntosCatetosOpuestos(nDiscret,n):
    paresDepuntos = []
    step = max(1,int(0.03 * n)) #cuanto mas bajo ete numero mejor si es 1 pixel va a tirar rayos en todas las lineas
...
tiemposCML = []
errores = []
vecPuntoUsadoEntrada = []
vecPuntoUsadoValor = []
for raizCantRayosSobre6 in range(int(nDiscret/2),2*nDiscret,10):
#for raizCantRayosSobre6 in [nDiscret]:
    vecEntradas.append(int(6 * (raizCantRayosSobre6**2)))
    paresDepuntos = generarPuntosRandom(raizCantRayosSobre6,n)
    ...
    ecm = errorCuadraticoMedio(imagen,leerCSV(csvSalida),nDiscret)
    errores.append(ecm)
    ...
    if(raizCantRayosSobre6 == nDiscret):
        vecPuntoUsadoEntrada.append(int(6 * (raizCantRayosSobre6**2)))
        vecPuntoUsadoValor.append(ecm)

#primer grafico: ecm
print(errores)
plt.figure(0)
plt.plot(vecEntradas,errores,'k-')
vecRojo = [2000]*len(vecEntradas)
vecAmarillo = [1000]*len(vecEntradas)
vecVerde= [200]*len(vecEntradas)
...
#plt.plot(vecPuntoUsadoEntrada,vecPuntoUsadoValor,'bo',label='utilizado en el metodo Random')
plt.legend()
plt.xlabel("Cantidad de rayos")
plt.ylabel("Error cuadratico medio")
plt.show()

#Segundo grafico : tiempos
# plt.figure(1)
# plt.plot(vecEntradas,tiemposTotales,'r-')
