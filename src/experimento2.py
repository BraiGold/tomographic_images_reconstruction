...
for ruido in range(1,1501,100):
    ...
    subprocess.call(['python3', './conversor_csv/csv_visualizer.py', csvSalida , './Exp1Resultados/'+'rec_'+nombreArchivoImagenSalida+'Ruido_'+str(ruido)+'_Ecm_'+str(int(ecmAprox))+'.png'])
    diffImagenes = toimage(leerCSV(csvSalida)-imagen, cmin=0.0, cmax=255)
    diffImagenes.save('./Exp1Resultados/'+'diff_' + nombreArchivoImagenSalida +'Ruido_'+str(ruido)+'_Ecm_'+str(ecmAprox)+'.png')
    vecECM.append(ecmAprox)
vecEntradas = [i for i in range(1,1501,100)]
plt.plot(vecEntradas, vecECM, 'k-')
# subprocess.call(['./main','-r',str(ruido), '-i' ,csvEntrada, '-s',csvSalida,'tipoRayo',str(4),'transpongoONO',str(trasponerONo),'usoMatrizEsparsaONO',str(0),'-n',str(nDiscret)])
